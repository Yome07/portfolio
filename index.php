...
$parcoursProsStatement = $db->prepare('SELECT *, CASE MONTH(debut)
        WHEN 1 THEN "janvier"
         ...
         ELSE "décembre"
        END  AS moisDebut
         , YEAR(debut) AS anneeDebut, CASE MONTH(fin) WHEN 1 THEN "janvier"
         WHEN 2 THEN "février"
         WHEN 3 THEN "mars"
         WHEN 4 THEN "avril"
         WHEN 5 THEN "mai"
         ...
         WHEN 11 THEN "novembre"
         ELSE "décembre"
        END AS moisFin, YEAR(fin) AS anneeFin FROM portfolio_parcoursPro ORDER BY debut DESC');
...
$realisationsStatement = $db->prepare('SELECT * FROM portfolio_realisation ORDER BY annee DESC');
...
<body>
  ...
  <section id="competences" class="d-flex py-2">
    <div class="container">
      <h1 class="border-bottom border-white border-2 pb-2">Compétences professionnelles</h1>
  ...
  <section id="parcours" class="d-flex py-2">
    <div class="container">
      <h1 class="border-bottom border-white border-2 pb-2">Parcours professionnel</h1>
      ...
      <article class="d-flex flex-column flex-lg-row align-items-stretch align-items-lg-start bgBlue bgWhite roundedDate">
        <p><?php if (($parcoursPro['anneeDebut'] == $parcoursPro['anneeFin']) && ($parcoursPro['moisDebut'] == $parcoursPro['moisFin'])) { 
            echo $parcoursPro['moisDebut']." ".$parcoursPro['anneeDebut'];
          } else {
            echo $parcoursPro['moisDebut']." ".$parcoursPro['anneeDebut']."<br/>".$parcoursPro['moisFin']." ".$parcoursPro['anneeFin'];
           } ?></p>
        <div>
  ...
  <section id="formations" class="d-flex py-2">
    <div class="container">
      <h1 class="border-bottom border-white border-2 pb-2">Formations</h1>
  ...
  <section id="realisations" class="d-flex py-2">
    <div class="container">
      <h1 class="border-bottom border-white border-2 pb-2">Réalisations</h1>
        ...
        <div class="card mb-2">
          ...
          <div class="card-body d-flex flex-column flex-nowrap">
            ...
            <?php if ($realisation['lien'] == null) { ?>
              <a href="#" target="_blank" class="btn btn-primary disabled mt-auto">Ce site n'est plus en ligne</a>
            <?php } elseif (preg_match("/github.com/", $realisation['lien']) == 1) {?>
              <a href="<?= $realisation['lien']; ?>" target="_blank" class="btn btn-primary mt-auto">Voir le projet sur github <i class="fa-solid fa-arrow-up-right-from-square"></i></a>
            <?php } else { ?>
              <a href="<?= $realisation['lien']; ?>" target="_blank" class="btn btn-primary mt-auto">Visiter le site <i class="fa-solid fa-arrow-up-right-from-square"></i></a>
            <?php } ?>
          </div>
        </div>
        <?php endforeach; ?>
  ...
  <!-- <script type="text/javascript" src="http://www.google.com/recaptcha/api/js/recaptcha_ajax.js"></script> -->
    <div class="container">
      ...
      <h2 class="text-center  m-3">Suivez-moi !</h2>
      ...
      <?php foreach ($utilisateurs as $utilisateur): ?>
      <p class="text-center m-3"> Télécharger mon CV au format pdf : <a href="<?= $utilisateur['cv']; ?>" target="_blank" class="link-light">cv.pdf <i class="fa-solid fa-file-pdf"></i></a></p>
      <?php endforeach; ?>  
    </div>
  </section>
  <footer class="text-center">
  <?php foreach ($hebergements as $hebergement): ?>
    <?= $hebergement['nomEditeur']; ?> | © 2022 | Tous droits réservés | <a data-bs-toggle="modal" data-bs-target="#infos" href="#infos">Mentions légales</a>
    <!-- Modal -->
